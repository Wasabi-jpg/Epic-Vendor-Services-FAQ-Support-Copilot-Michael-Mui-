...
def answer_with_sources(
    ...
    Returns: {"answer": str, "sources": [{"category", "question", "source_url"}], "model_used": str}
    """
    import ollama
    ...
    system = """You are an Epic Vendor Services support agent. Answer only using the provided FAQ context. Be concise. If the context does not contain enough information, say so and suggest the user rephrase or contact support. For login or password issues, direct users to use "Forgot username or password" or contact their admin / UserWeb Support / vendorservices@epic.com—do not invent reset steps."""
...
def run_query(
    ...
    if faq_data is None:
        faq_data = load_faq()
    retrieved = retrieve(query, faq_data=faq_data, top_k=top_k)
